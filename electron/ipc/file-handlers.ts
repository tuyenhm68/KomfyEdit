...
/**
 * Base64 inflates by a third and the whole thing lands in one string, so this is
 * only viable for small files. Anything larger should stream instead — see
 * `getAudioPeaks` for the waveform case.
 */
const MAX_READ_LOCAL_FILE_BYTES = 64 * 1024 * 1024

function readLocalFileAsBase64(filePath: string): { data: string; mimeType: string } {
  const { size } = fs.statSync(filePath)
  if (size > MAX_READ_LOCAL_FILE_BYTES) {
    throw new Error(
      `File is too large to read in one piece (${(size / 1024 / 1024).toFixed(0)} MB, limit `
      + `${MAX_READ_LOCAL_FILE_BYTES / 1024 / 1024} MB)`,
    )
  }
...
export function registerFileHandlers(): void {
  ...
  handle('getLutContent', async ({ filterId }) => {
    try {
      const { resolveLutPath } = await import('../export/lut-filter')
      const lutPath = resolveLutPath(filterId)
      if (!fs.existsSync(lutPath)) {
        return { success: false, error: `LUT file not found: ${filterId}` }
      }
