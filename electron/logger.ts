...
function log(level: LogLevel, consoleMethod: 'log' | 'warn' | 'error', message: string): void {
  // When running inside an MCP stdio server process, stdout is strictly reserved
  // for JSON-RPC messages. Never write diagnostic logs to stdout.
  const effectiveMethod = process.env.KOMFYEDIT_MCP_PROFILE && consoleMethod === 'log'
    ? 'warn'
    : consoleMethod
  console[effectiveMethod](message)
