...
/** Decode in the browser. Only used for non-file URLs, which are small by nature. */
async function computeWaveformInRenderer(url: string, buckets: number): Promise<Float32Array> {
...
/**
 * Amplitude envelope for a clip's audio.
 *
 * Local media goes through the main process, where ffmpeg streams a low-rate
 * decode and reduces it to peaks. Reading the file into the renderer instead
 * would need the whole thing in memory as base64 — that throws outright past
 * 2 GB and is punishing well before it.
 */
export async function computeWaveform(url: string, buckets: number = 800): Promise<Float32Array> {
  ...
  try {
    const peaks = url.startsWith('file://') && window.electronAPI?.getAudioPeaks
      ? Float32Array.from(await window.electronAPI.getAudioPeaks({ filePath: url, buckets }))
      : await computeWaveformInRenderer(url, buckets)
...
interface ClipWaveformProps {
  ...
  color?: string
  /**
   * The window of source media the clip currently shows. Without it the whole
   * envelope is stretched across the clip, so trimming appears to squeeze the
   * waveform instead of cropping it. Omit for callers that really do want the
   * entire file drawn edge to edge.
   */
  sourceDuration?: number
  trimStart?: number
  /** Timeline duration of the clip; combined with speed to find the out-point. */
  duration?: number
  speed?: number
}
...
export function ClipWaveform({
  ...
  // A video with no audio track yields an all-zero envelope; drawing a flat line
  // across the clip would just be noise, so render nothing at all.
  const isSilent = peaks !== null && !peaks.some(peak => peak > 0.002)

  useEffect(() => {
    if (!url) return
    let cancelled = false
    // Use the shared default rather than a coarse 200: a trimmed clip only
    // shows a slice of these peaks, and 200 across a whole file leaves too few
    // for that slice to read as a waveform.
    computeWaveform(url).then(p => {
  ...
  const draw = useCallback(() => {
    ...
    // Map pixels onto the slice of source the clip actually plays, so trimming
    // slides a window over a fixed envelope instead of rescaling it.
    let startFraction = 0
    let endFraction = 1
    if (sourceDuration && sourceDuration > 0 && duration && duration > 0) {
      startFraction = Math.max(0, Math.min(1, trimStart / sourceDuration))
      endFraction = Math.max(startFraction, Math.min(1, (trimStart + duration * speed) / sourceDuration))
      // A degenerate window would draw nothing; fall back to the whole file.
    ...
    const ampAt = (i: number): number => {
      const fraction = startFraction + (i / w) * span
      const peakIdx = Math.floor(fraction * peaks.length)
      return peaks[Math.max(0, Math.min(peakIdx, peaks.length - 1))]
    }

    ctx.fillStyle = color
    ctx.beginPath()
    for (let i = 0; i < w; i++) {
      const y = centerY - ampAt(i) * maxAmp
      if (i === 0) ctx.moveTo(i, y)
      else ctx.lineTo(i, y)
    }
    for (let i = w - 1; i >= 0; i--) {
      ctx.lineTo(i, centerY + ampAt(i) * maxAmp)
    }
    ctx.closePath()
    ctx.fill()
  }, [peaks, color, sourceDuration, trimStart, duration, speed])
